...
const JobManage = () => {
  ...
  const handleJobSubmit = async (data) => {
    try {
      console.log("job ");
      axios.post(`${AdminApi}jobPosition`,{data:data, token :admin}).then((res)=>{
          if(refresh===true){
              setRefresh(false)
          }else{
            setRefresh(true)
  ...
  useEffect(() => {
    const showJobManagement = async ()=>{
      try {
      axios.get(`${AdminApi}viewJobManage?data=${encodeURIComponent(admin)}`).then((res)=>{
  ...
  return (
    <section className="h-full overflow-scroll">
      ...
      <Button className="p-1 w-20 ml-5 border border-transparent text-white rounded bg-pink-500 shadow-md hover:bg-pink-400"
       onClick={showModal}>
        New Job
      </Button>
      <Modal
      ...
      >
        ...
        >
          ...
          {errors.job && errors.job.type === "required" && (
            <label className="ml-24 text-sm text-red-600">Please enter the job</label>
          )}
